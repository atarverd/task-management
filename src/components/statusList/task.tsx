...
export const Task = ({title,boardTitle,id}:TProp) => {
  ...
  return (
    <Box bg={colorMode === 'light'?'gray.300':'gray.900'} borderRadius='8px' p='8px' my='24px' mx='10%'>
        <Text p='5px'>{title}</Text>
        <ModalInput boardId={boardId} boardName={boardTitle} id={id} title={title}/>
        
        <Button ml='5px' size='sm' colorScheme='red' onClick={()=>dispatch(deleteTask({boardId,boardTitle,id}))}>Delete</Button>
    </Box>
  )
}
